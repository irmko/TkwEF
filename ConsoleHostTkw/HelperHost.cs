...
namespace ConsoleHostTkw
{
    class HelperHost
    {
        ...
        public static Uri GetAddress(bool test, string suffix)
        {
            ...
            if (test)
            {
                return new Uri(string.Format("{0}{1}{2}", "http://localhost:9001", (suffix.Trim().Length > 0 ? "/" : ""), suffix.Trim()));
            }
            else
            {
                return new Uri(string.Format("{0}{1}{2}", "net.tcp://localhost:9002", (suffix.Trim().Length > 0 ? "/" : ""), suffix.Trim()));
            }
        }
